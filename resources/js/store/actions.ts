	...
	async ({ dispatch }) => {
		...
		try {
			...
			const error = await HarborError.wrap(
				...
				__(
					'Liquid Web failed to enable your feature.',
					'%TEXTDOMAIN%'
				)
			);
	...
	async ({ dispatch }) => {
		...
		try {
			...
			const error = await HarborError.wrap(
				...
				__(
					'Liquid Web failed to disable your feature.',
					'%TEXTDOMAIN%'
				)
			);
			dispatch({ type: 'TOGGLE_FEATURE_FAILED', slug, error });
	...
	async ({ dispatch }) => {
		...
		try {
			...
			const error = await HarborError.wrap(
				...
				__(
					'Liquid Web failed to update your feature.',
					'%TEXTDOMAIN%'
				)
			);
	...
	async ({ dispatch, select }) => {
		if (!select.canModifyLicense()) {
			return new HarborError(
				...
				__(
					'Liquid Web failed to activate your license, another action is in progress.',
					'%TEXTDOMAIN%'
				)
			);
		}
		...
		try {
			...
			const error = await HarborError.wrap(
				...
				__(
					'Liquid Web failed to activate your license.',
					'%TEXTDOMAIN%'
				)
			);
	...
	async ({ dispatch, select }) => {
		if (!select.canModifyLicense()) {
			return new HarborError(
				ErrorCode.LicenseActionInProgress,
				__(
					'Liquid Web failed to delete your license, another action is in progress.',
					'%TEXTDOMAIN%'
				)
			);
		}
		...
		try {
			...
			const error = await HarborError.wrap(
				...
				__(
					'Liquid Web failed to remove your license.',
					'%TEXTDOMAIN%'
				)
			);
