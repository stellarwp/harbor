...
The gate is a compliance boundary, not just a performance optimization. Most entry plugins are distributed on WordPress.org, where a plugin may not validate license keys, make outbound calls the user never opted into, or install executable code served from outside the .org repository. Several of Harbor's capabilities could be read as running against those guidelines if a plugin distributed there exercised them, so the gate keeps a free-only site from doing any of them. Only the global-function registry loads unconditionally, so cross-copy version negotiation still works while everything networked stays off.
