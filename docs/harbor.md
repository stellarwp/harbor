...
The gate is a compliance boundary, not just a performance optimization. Most entry plugins are distributed on WordPress.org, where a plugin may not validate license keys, make outbound calls the user never opted into, or install executable code served from outside the .org repository. The gate is what keeps a free-only site from doing any of those things — when Harbor first shipped inside The Events Calendar, the WordPress.org plugins team threatened to remove the plugin. Only the global-function registry loads unconditionally, so cross-copy version negotiation still works while everything networked stays off.
